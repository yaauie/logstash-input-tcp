...
** `v1`: structured connection metadata added under `[@metadata][tcp]`
...
|[host]                  |[@metadata][tcp][source][name] |Always       |Name or address of source (see <<plugins-{type}s-{plugin}-dns_reverse_lookup_enabled>>)
|[@metadata][ip_address] |[@metadata][tcp][source][ip]   |Always       |IP address of source
|[port]                  |[@metadata][tcp][source][port] |Always       |Outgoing TCP port of source (typically random when operating in `mode => server`)
|[proxy_host]            |[@metadata][tcp][proxy][ip]    |When `proxy_protocol => true` |IP address of the proxy that initially received the TCP stream.
|[proxy_port]            |[@metadata][tcp][proxy][port]  |When `proxy_protocol => true` |The port on the proxy that initially received the TCP stream.
|[sslsubject]            |[@metadata][tcp][ssl][subject] |When `ssl_enable => true` AND `ssl_verify => true` |The peer certificate's SSL Subject
|=======================================================================
...
-----
filter {
  if [@metadata][tcp][source] and not [host] {
    mutate {
      copy {
        "[@metadata][tcp][source][name]" => "[host][name]"
        "[@metadata][tcp][source][ip]"   => "[host][ip]"
      }
    }
  }
}
