...
describe LogStash::Inputs::Tcp do
  ...
  [:disabled,:v1].each do |ecs_compatibility|
    context "with `ecs_compatibility => `#{ecs_compatibility}`" do
      before(:each) do
      ...
      it "should read plain with unicode" do
        ...
        event_count.times do |i|
          ...
          when :v1
            insist { ["localhost","ip6-localhost"].includes? event.get("[@metadata][tcp][source][name]") }
            insist { event.get("[@metadata][tcp][source][ip]") } == '127.0.0.1'
          end
        end
      end
      ...
      it "should handle PROXY protocol v1 connections" do
        ...
        event_count.times do |i|
          ...
          when :disabled
            ...
            insist { events[i].get("[@metadata][ip_address]") } == "1.2.3.4"
            insist { events[i].get("port") } == "1234"
            insist { events[i].get("proxy_host") } == "5.6.7.8"
            insist { events[i].get("proxy_port") } == "5678"
          when :v1
            insist { events[i].get("message") } == "#{i} ☹"
            insist { events[i].get("[@metadata][tcp][source][name]") } == "1.2.3.4"
            insist { events[i].get("[@metadata][tcp][source][ip]") } == "1.2.3.4"
            insist { events[i].get("[@metadata][tcp][source][port]") } == "1234"
            insist { events[i].get("[@metadata][tcp][proxy][ip]") } == "5.6.7.8"
            insist { events[i].get("[@metadata][tcp][proxy][port]") } == "5678"
          end
        end
      end
      ...
      it "should read events with json codec (testing 'host' handling)" do
        ...
        when :v1
          insist { event }.include?("[@metadata][tcp][source][name]")
          insist { event }.include?("[@metadata][tcp][source][ip]")
        end
      end
